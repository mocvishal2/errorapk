...
class NotifikasiScreenPenjual extends StatefulWidget {
  final User? user;

  @override
  NotifikasiScreenPenjual({required this.user});
  _NotifikasiScreenPenjualState createState() =>
      _NotifikasiScreenPenjualState();
}

class _NotifikasiScreenPenjualState extends State<NotifikasiScreenPenjual> {
  ...
  void _clearCartData(String sellerName) async {
    try {
      ...
      QuerySnapshot cartSnapshot = await FirebaseFirestore.instance
          .collection('users')
          .doc(uid)
          .collection('notifikasi_penjual')
          .doc('pembeli')
          .collection(uid)
          .where('idpembeli', isEqualTo: sellerName)
          .get();
  ...
  void _fetchNotifikasiData() async {
    try {
      ...
      for (var notifikasi in notifikasi) {
        String sellerName = notifikasi['namapembeli'];
                ...
                return Column(
                  ...
                  children: [
                        ...
                        return ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text(
                                  'Total Harga untuk pembeli: Rp${_calculateTotalPriceForPenjual(notifikasi['idpenjual'])}'),
                    ...
                    ElevatedButton(
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (context) {
                            return AlertDialog(
                              title: Text(
                                  'Deta belanjaan dari pembeli $notifikasiName '),
                                  ...
                                  children: notifikasiProducts.map((notifikas) {
                                    return Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            'Nama pembeli: ${notifikas['namapembeli']}'),
                                        Text(
                              ...
                              actions: [
                                ...
                                Text(
                                    'Sebelum hapus notifikasi tolong di screenshot agar tidak lupa'),
                                TextButton(
                                  onPressed: () async {
                                    User? user =
                      ...
                      child: Column(
                        children: [
                          Text('Data dari pembeli $notifikasiName'),
                          Text('Total Harga: Rp$sellerTotalPrice'),
                        ],
                      ),
                    ),
